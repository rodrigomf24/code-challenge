var HouseholdObjects = {
  ...
  put:function(id, params) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      _this.executeQuery('UPDATE household_objects SET '+updates.join(', ')+' WHERE id = \''+id+'\' RETURNING id;').then(function(response) {
        if(response.rows !== void(0) && response.rows.length > 0) {
          resolve(response.rows);
        } else {
  ...
  delete:function(id) {
    var _this = this;
    return new Promise(function(resolve, reject) {
      _this.executeQuery('DELETE FROM household_objects WHERE id = \''+id+'\';').then(function(response) {
...
module.exports = HouseholdObjects;