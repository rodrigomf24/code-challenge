...
var Vehicle = {
  ...
  put:function(id, params) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      _this.executeQuery('UPDATE vehicle SET '+updates.join(', ')+' WHERE id = \''+id+'\' RETURNING id;').then(function(response) {
        if(response.rows !== void(0) && response.rows.length > 0) {
          resolve(response.rows);
        } else {
  ...
  delete:function(id) {
    var _this = this;
    return new Promise(function(resolve, reject) {
      _this.executeQuery('DELETE FROM vehicle WHERE id = \''+id+'\'; DELETE FROM household_objects WHERE object_id =\''+id+'\';').then(function(response) {
