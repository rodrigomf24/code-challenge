...
var householdModel = require('./../models/household');

/* GET household listing. */
router.get('/', function(req, res, next) {
	console.log('WITHOUT ID');
	householdModel.get.all().then(function(response) {
		res.json(response);
	}, function(err) {
		res.status(404).json({
			error:err
...
router.get('/:id', function(req, res, next) {
	...
	if(req.params.id === void(0)) {
		...
		householdModel.get.single(req.params.id).then(function(response) {
			res.json(response);
		}, function(err) {
			res.status(404).json({
				error:err
...
var validateParameters = function(params, type) {
	return new Promise(function(resolve, reject) {
		var requiredFields = ['address', 'zip', 'city', 'state', 'bedrooms_number'],
			valid = true,
			values = [],
			count=0;
	...
	if(typeof(req.body) === 'object' && Object.keys(req.body).length > 0) {
		validateParameters(req.body, 'post').then(function(response) {
			if(!response.valid) {
				...
				householdModel.post(response.values).then(function(response) {
	...
	if(typeof(req.body) === 'object' && Object.keys(req.body).length > 0 && 'id' in req.body) {
		...
		validateParameters(req.body, 'put').then(function(response) {
			if(!response.valid) {
				...
				householdModel.put(id, response.values).then(function(response) {
					req.body.id = response[0].id;
					res.json(req.body);
				}, function(err) {
					res.status(404).json({
...
router.delete('/:id', function(req, res, next) {
	if(req.params.id === void(0)) {
		...
		householdModel.delete(req.params.id).then(function(response) {
